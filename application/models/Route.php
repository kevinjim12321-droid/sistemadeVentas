...
class Route extends MY_Model
{
	...
	/**
	 * One-time cleanup for warehouse receivings that were meant to be route
	 * purchases (the early "Comprar para esta ruta" attempts that failed and
	 * fell back to a normal receiving). Only reverses a receiving when every
	 * lot it created is still completely untouched, so it can never drive
	 * inventory negative. Returns a per-receiving report.
	 */
	public function reverse_stray_bodega_receivings(array $receiving_ids, $employee_id)
	{
		$report = array();
		foreach ($receiving_ids as $receiving_id)
		{
			$receiving_id = (int)$receiving_id;
			$recv = $this->db->get_where('receivings', array('receiving_id' => $receiving_id))->row();
			if (!$recv)
			{
				$report[$receiving_id] = 'no existe';
				continue;
			}
			if ((int)$recv->deleted === 1)
			{
				$report[$receiving_id] = 'ya estaba borrado';
				continue;
			}
			if (!empty($recv->route_id))
			{
				$report[$receiving_id] = 'omitido: sí pertenece a una ruta';
				continue;
			}

			$lots = $this->db->get_where('inventory_lots', array('receiving_id' => $receiving_id))->result();
			$touched = FALSE;
			foreach ($lots as $lot)
			{
				if (abs((float)$lot->quantity_remaining - (float)$lot->quantity_initial) > 0.0000000001)
				{
					$touched = TRUE;
				}
			}
			if ($touched)
			{
				$report[$receiving_id] = 'omitido: algún lote ya se movió/vendió';
				continue;
			}
			...
			foreach ($lots as $lot)
			{
				...
				if ($lot->item_variation_id)
				{
					$this->db->set('quantity', 'quantity-'.$this->decimal($qty), FALSE);
					$this->db->where('item_variation_id', (int)$lot->item_variation_id);
					$this->db->where('location_id', (int)$lot->location_id);
					$this->db->update('location_item_variations');
					$current = $this->db->get_where('location_item_variations', array('item_variation_id' => (int)$lot->item_variation_id, 'location_id' => (int)$lot->location_id))->row();
				}
				else
				{
					$this->db->set('quantity', 'quantity-'.$this->decimal($qty), FALSE);
					...
					$this->db->where('location_id', (int)$lot->location_id);
					$this->db->update('location_items');
					$current = $this->db->get_where('location_items', array('item_id' => (int)$lot->item_id, 'location_id' => (int)$lot->location_id))->row();
				}
				...
				$this->db->insert('inventory', array(
					...
					'item_variation_id' => $lot->item_variation_id ? (int)$lot->item_variation_id : NULL,
					'trans_user' => (int)$employee_id,
					'trans_comment' => 'REVERSA RECV '.$receiving_id.' (compra de ruta fallida) | Lote: '.$lot->lot_code,
					'trans_inventory' => $this->decimal(-$qty),
					'location_id' => (int)$lot->location_id,
					'trans_current_quantity' => $this->decimal($current ? $current->quantity : 0),
				));

				$this->db->delete('inventory_lot_movements', array('lot_id' => (int)$lot->lot_id));
				$this->db->delete('inventory_lots', array('lot_id' => (int)$lot->lot_id));
			}

			$this->db->delete('receivings_payments', array('receiving_id' => $receiving_id));
			$this->db->where('receiving_id', $receiving_id);
			$this->db->update('receivings', array('deleted' => 1, 'deleted_by' => (int)$employee_id, 'last_modified' => $now));

			if ($this->db->trans_status() === FALSE)
			{
				$this->db->trans_rollback();
				$report[$receiving_id] = 'ERROR: no se pudo revertir (sin cambios)';
				continue;
			}
			$this->db->trans_commit();
			$report[$receiving_id] = 'revertido: -'.rtrim(rtrim(number_format($removed_units, 4, '.', ''), '0'), '.').' unidades de bodega, pago y recibo anulados';
		}
		return $report;
	}
}
