...
<head>
    ...
    <title><?php 
		 ...
		 echo !is_on_demo_host() ?  $company.' -- '.lang('common_powered_by').' '.$this->config->item('branding')['name'] : 'Demo - '.$this->config->item('branding')['name'].' | Punto de Venta' ?></title>
		...
		<div class="left-bar hidden-print" >
			...
			<ul class="list-unstyled menu-parent" id="mainMenu">
				...
				foreach($allowed_modules->result() as $module) { 
					if (array_search('customers', $disable_modules) === false || array_search('suppliers', $disable_modules) === false) {
					
						if ($module->module_id == 'customers' || $module->module_id == 'suppliers') {
							$parties = true;
							$active_class = '';
							if ($this->uri->segment(1) == 'customers' || $this->uri->segment(1) == 'suppliers') {
								$active_class = 'show';
							}
						}
					}
				...
				if ($parties == true) { ?>
					<li class="has_sub_menu">
						...
						<a tabindex = "-1" href=""  class="waves-effect waves-light" data-toggle="collapse" aria-expanded="false">
							...
							<span class="text"><?php echo lang("common_contacts") ?></span>
							<span class="dropdown pull-right-container">
                    <i class="icon ti-angle-down"></i>
                </span>
						</a>
					<ul class="collapse list-unstyled <?php echo $active_class ? $active_class :'' ?>" >
						<?php if ($this->Employee->has_module_permission('customers', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
							<li <?php echo array_search('customers', $disable_modules) === false ? '': 'style="display: none;"' ?>>
								<a href="<?php echo site_url('customers'); ?>"><i class="icon ti-user"></i><?php echo lang('reports_customers')?></a>
							</li>
						<?php } ?>
						<?php if ($this->Employee->has_module_permission('suppliers', $this->Employee->get_logged_in_employee_info()->person_id)) { ?>
							<li <?php echo array_search('suppliers', $disable_modules) === false ? '': 'style="display: none;"' ?>>
								<a href="<?php echo site_url('suppliers'); ?>"><i class="icon ti-download"></i><?php echo lang('reports_suppliers')?></a>
							</li>
						<?php } ?>
					</ul>
				</li>
				...
				<?php } elseif ($module->module_id == 'invoices') { ?>
					<li class="has_sub_menu">
				...
				<?php } elseif ($module->module_id == 'items') { ?>

				<?php } elseif($module->module_id != 'item_kits' && $module->module_id != 'customers' && $module->module_id != 'suppliers' && $module->module_id != 'price_rules') {?>
