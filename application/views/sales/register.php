...
<div class="alert alert-info" style="margin:10px 15px 0"><strong>Venta de ruta:</strong> <?php echo H($this->cart->route_name); ?>. El producto se descontará del inventario cargado en esta ruta. <span class="pull-right"><?php if (!$this->route_blocked && $this->cart->get_mode() !== 'store_account_payment') { echo anchor('sales/change_mode/store_account_payment/1', 'Cobrar abono de cliente'); ?> &nbsp;|&nbsp; <?php } echo anchor('routes/view/'.$this->cart->route_id, 'Volver a la ruta'); ?><?php if (!$this->route_blocked) { ?> &nbsp;|&nbsp; <?php echo anchor('sales/clear_route', 'Salir de la ruta'); ?><?php } ?></span></div>
<?php } ?>
<?php if ($this->route_blocked) { ?>
<div class="alert alert-danger" style="margin:10px 15px 0"><strong><?php echo lang('sales_route_blocked_title'); ?></strong> <?php echo H($this->route_blocked_reason); ?> <?php echo lang('sales_route_blocked_hint'); ?></div>
<?php } ?>
<script>
									...
									<tbody class="register-item-content" data-line="<?php echo $line; ?>">
										...
										<tr class="register-item-bottom">
											...
											<td colspan="5">
												...
												<?php 
													if($supplier_id && !$this->config->item('hide_supplier_on_sales_interface')){
		...
		<?php if (isset($customer)) { ?>
			...
			<div class="customer-badge">
				...
				<div class="details">
					...
					{
					?>
						<?php echo anchor("customers/quick_modal/$customer_id/1", '<i class="ion-ios-compose-outline"></i>',  array('id' => 'edit_customer', 'data-toggle'=>"modal", 'data-target'=>"#myModalDisableClose", 'class' => 'btn btn-edit btn-primary pull-right', 'title' => lang('common_update'))) . ''; ?>
						...
						buttons: {
							...
							api_ig: {
								...
								callback: function(){
									...
									$("#item").autocomplete('option', 'source', '<?php echo site_url("items/sync_ig_item_search"); ?>');
									...
									$("#item").autocomplete('option', 'response', 
										function(event, ui){
											...
											}else if(ui.content.length == 0 && (source_url.indexOf('items/sync_ig_item_search') > -1)){
												var noResult = {
													value:"",
							...
							api_wgp: {
								...
								callback: function(){
									...
									$("#item").autocomplete('option', 'source', '<?php echo site_url("items/sync_wgp_inventory_search"); ?>');
									...
									$("#item").autocomplete('option', 'response', 
										function(event, ui){
											...
											}else if(ui.content.length == 0 && (source_url.indexOf('items/sync_wgp_inventory_search') > -1)){
												var noResult = {
													value:"",
													image:"<?php echo base_url()."assets/img/item.png"; ?>",
