		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="workorder-badge">
				...
				<ul class="list-inline pull-right">
					<?php
					if ($this->Location->get_info_for_key('blockchyp_work_order_pre_auth'))
					{
					?>
					...
					<?php
					if ($this->Location->get_info_for_key('blockchyp_work_order_post_auth'))
					{
					?>
			...
			<div class="panel panel-piluku item_being_repaired_info">
				...
				<div class="panel-body">
					...
					<dl class="dl-horizontal">
						...
						<dd><?php echo clean_html($item_being_repaired_info['description']); ?></dd>
...
<script type='text/javascript'>
			...
			$('#item').bind('keypress', function(e) {
				if(e.keyCode==13)
				{
					...
					$.get('<?php echo site_url("work_orders/item_search");?>', {term: $("#item").val()}, function(response)
					{
						...
						else if(data.length == 0 && <?php echo count($vendor_list) > 0? 1 : 0 ?>){
							...
							setTimeout(function(){
								...
								if(search_item_key.trim() != ""){
									...
									bootbox.dialog({
										...
										buttons: {
											<?php
											if( in_array('ig_api_bearer_token', $vendor_list)){
											?>
											api_ig: {
											...
											if( in_array('wgp_integration_pkey', $vendor_list)){
											?>
											api_wgp: {
												label: 'WGP',
												className: 'btn-info',
	...
	$('input[id^="files_"]').change(function(){
		auto_save_form();
	});
