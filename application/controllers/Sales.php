...
class Sales extends Secure_area
{
  ...
  function cancel_sale()
  {
	...
	function return_order($order_id){
		$sale_prefix = strtolower($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS');
		$order_id = $sale_prefix.' '.$order_id;
		
		$this->cart->destroy();
		$this->cart->set_mode('return');

		if($this->cart->is_valid_receipt($order_id) && $this->cart->get_mode()=='return'){
			$this->cart->process_barcode_scan($order_id);
			redirect('sales');
		}
		return;
	}
