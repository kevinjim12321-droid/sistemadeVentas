	...
	*/
	...
	function save($item_id=-1)
	{
		...
		if($this->Item->save($item_data,$item_id))
		{
			...
			{
				$this->Item_serial_number->save($item_id, $this->input->post('serial_numbers'), $this->input->post('serial_number_cost_prices'), $this->input->post('serial_number_prices'),$this->input->post('serial_number_prices_variations'),$this->input->post('serials_to_delete'), $this->input->post('add_to_inventory'));
			}
			else
			{
	...
	function sync_ig_bestsellers(){
		...
		if(ENVIRONMENT == "development"){
			// dev
			// $url = "http://202.131.112.74/injuredg/rest/V1/bestsellers";
		...
		if(isset($bestseller_list->message) && (strpos($bestseller_list->message, "The consumer isn't authorized to access") > -1)){
			...
			foreach($bestseller_list as $k => $item_info){
				...
				if($new_item == 1){
					...
					if(ENVIRONMENT == "development"){
						// dev
						// $item_image_link = "http://202.131.112.74/injuredg/pub/media/catalog/product".$item_image;
	...
	function sync_ig_item_search(){
		...
		if(ENVIRONMENT == "development"){
			// $url = "http://202.131.112.74/injuredg/rest/V1/pos/products?searchCriteria[filterGroups][0][filters][0][field]=sku&searchCriteria[filterGroups][0][filters][0][value]=".$search_key."&searchCriteria[filterGroups][0][filters][0][conditionType]=eq&searchCriteria[pageSize]=20&searchCriteria[currentPage]=1";
		...
		if(isset($ret_array->message) && (strpos($ret_array->message, "The consumer isn't authorized to access") > -1)){
			...
			if(count($ret_array) == 0){
				//upc search
				if(ENVIRONMENT == "development"){
					// $url = "http://202.131.112.74/injuredg/rest/V1/pos/products?searchCriteria[filterGroups][0][filters][0][field]=upc&searchCriteria[filterGroups][0][filters][0][value]=".$search_key."&searchCriteria[filterGroups][0][filters][0][conditionType]=eq&searchCriteria[pageSize]=20&searchCriteria[currentPage]=1";
			...
			foreach($ret_array as $item_info){
				...
				$item_data2 = array(
					...
					'cost_price'=>$item_data1->cost_price,
					'unit_price'=>$item_data1->unit_price,
					'label' => $item_data1->product_id." (".$item_data1->name.") - ".$item_data1->unit_price,
	...
	function sync_wgp_inventory(){
		...
		$url = "https://api.wgp.com/wgpjson.aspx?reqtype=get-inventory&pkey=".$url_parameter;
		$curl = curl_init($url);
		curl_setopt($curl, CURLOPT_URL, $url);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
		...
		if(count($inventory_list) == 0 ){
			...
			foreach($inventory_list as $k => $item_info){
				...
				$item_data = array(
					...
					'product_id'=> $item_info->SKU,
					'cost_price'=>$item_info->cost_price,
					'unit_price'=>$item_info->cost_price,
				);
				...
				if($new_item == 1){
					...
					if($this->config->item('wgp_integration_userid')){
						$url_parameter = rawurlencode($this->config->item('wgp_integration_pkey'));
						$url_userid = rawurlencode($this->config->item('wgp_integration_userid'));
						$url = "https://api.wgp.com/wgpjson.aspx?reqtype=user-sku-price&wgpcustid=".$url_userid."&sku=".$item_info->SKU."&pkey=".$url_parameter;
						$curl = curl_init($url);
						curl_setopt($curl, CURLOPT_URL, $url);
						curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
						...
						$find_item = json_decode($resp, true);

						$this->set_supplier_item("WGP", $search_item_id);

						if( $find_item['username'] == $this->config->item('wgp_integration_userid') && $item_data['cost_price'] != $find_item['cost_price']){
							$item_data['cost_price'] = $find_item['tier_price'];
							$item_data['unit_price'] = $find_item['tier_price'];
							$this->Item->save($item_data, $search_item_id);
						}
					}
				}
	...
	function sync_wgp_inventory_search(){
		...
		if(count($inventory_list) == 0){
			echo json_encode(array('success'=>false,'message'=>lang('items_sync_wgp_inventory_failed')));
		}else{
			$search_item_list = array();

			foreach($inventory_list as $item_info){

				if($item_info['UPC'] == $search_key || $item_info['SKU'] == $search_key){
					...
					if($new_item == 1){
						...
						if($this->config->item('wgp_integration_userid') && $item_info['SKU'] == $search_key){
							$url_parameter = rawurlencode($this->config->item('wgp_integration_pkey'));
							$url_userid = rawurlencode($this->config->item('wgp_integration_userid'));
							...
							if( $find_item['username'] == $this->config->item('wgp_integration_userid') && $item_data['cost_price'] != $find_item['tier_price']){
								$item_data['cost_price'] = $find_item['tier_price'];
								$item_data['unit_price'] = $find_item['tier_price'];
								$this->Item->save($item_data, $search_item_id);
							}
