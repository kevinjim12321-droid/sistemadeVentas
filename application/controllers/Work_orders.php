		...
		if(count($status_changed_work_order_ids) > 0){
			foreach($status_changed_work_order_ids as $work_order_id)
			{
				...
				if($work_order_status_info->notify_by_email || $work_order_status_info->notify_by_sms){
					...
					$status_template = $this->status_email_template($status, $work_order_info, $work_order_status_info);
					...
					if($work_order_status_info->notify_by_email){
						...
						if($customer_email){
							...
							$this->Common->send_email($customer_email,$subject,$message,$status_template['images']);
						}
					}
	...
	function status_email_template($status_id, $work_order_info, $work_order_status_info)
	{
		...
		if ($status_template) {
			...
			$search_replace = array(
								...
								"%estimated_repair_date%"	=> date('Y-m-d H:i:s', strtotime($work_order_info->estimated_repair_date)),
